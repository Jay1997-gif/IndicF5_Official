...
import shutil
import tempfile
from f5_tts.infer.frontend import preprocess
...
from f5_tts.infer.utils_infer import (preprocess_gen_text,
    load_vocoder,
    load_model,
...
def load_f5tts(ckpt_path=str(cached_path("hf://SWivid/F5-TTS/F5TTS_Base/model_1200000.safetensors"))):
    ...
    ckpt_path = "runs/indic_5/model.safetensors"
    vocab_path = "runs/indic_5/vocab.txt"
    ...
    checkpoint = load_file(ckpt_path, device=my_device)
...
def infer(
    ...
    shutil.copy(
        ...
        os.path.join(
            "runs/indic_5/infers",
            f"reference_audio_{current_time}.wav",
        ),
    )
    ...
    # -------- Text preprocessing --------
    gen_text = preprocess_gen_text(gen_text)

    from f5_tts.infer.frontend import preprocess

    gen_text = preprocess(gen_text)
    print("FINAL TEXT:", gen_text)
    ...
    sf.write(
        os.path.join(
            "runs/indic_5/infers",
            f"reference_audio_{current_time}_gen_{gen_time}.wav",
        ),
        final_wave,
        final_sample_rate,
    )
